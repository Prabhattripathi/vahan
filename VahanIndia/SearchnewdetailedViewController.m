//
//  SearchnewdetailedViewController.m
//  VahanIndia
//
//  Created by geniemac4 on 30/04/15.
...
#import "SearchnewdetailedTableViewCell.h"
//#import "ByPriceFinalViewController.h"


@interface SearchnewdetailedViewController ()
{
    UIImage * image;
    ...
    NSString*brandstr ,*modelstr,*vehical,*budget;
}

@end

@implementation SearchnewdetailedViewController
...
- (void)viewDidLoad
{
    ...
    [HUD showAnimated:YES whileExecutingBlock:^{
        ...
        [self GetNewCarSearchData];
        
         [self GetUsedCarSearchData];
...
-(void)GetNewCarSearchData

{
    if (app.test==true)
    {
        budget=[NSString stringWithFormat:@"%@",app.budgetstr];
        NSLog(@"budget%@",budget);
        vehical=[NSString stringWithFormat:@"%@",app.vehicaltype];
        brandstr=@"";
        modelstr=@"";
    ...
    if (app.test==false)
    {
        brandstr=[NSString stringWithFormat:@"%@",app.brandstr];
        modelstr=[NSString stringWithFormat:@"%@",app.modelstr];
        budget=@"";
        vehical=@"";
    ...
    NSString *urlString =[NSString stringWithFormat:@"http://demo1.geniesoftsystem.com/vahanindia/api/index.php/api/Quicksearchnew?data={\"price\":\"%@\",\"bodytype\":\"%@\",\"brand\":\"%@\",\"model\":\"%@\"}",budget,vehical,brandstr,modelstr];
    NSLog(@"urlstuyg%@",urlString);
    
    //http://demo1.geniesoftsystem.com/vahanindia/api/index.php/api/Quicksearchnew?data={%22price%22:%2220-lakh-35-lakh%22,%22bodytype%22:%22luxury%22,%22brand%22:%22%22,%22model%22:%22%22}
    ...
    respdic = [jsonObjects valueForKey:@"Newcarsearch"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *simpleTableIdentifier = @"SearchnewdetailedTableViewCell";
    
    SearchnewdetailedTableViewCell *cell1 = (SearchnewdetailedTableViewCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
    
    if (cell1 == nil)
    {
        NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"SearchnewdetailedTableViewCell" owner:self options:nil];
    ...
    cell1.lbl_showrumprice.text=[array6 objectAtIndex:indexPath.row];
